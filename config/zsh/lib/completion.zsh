...
# Format group matches in completion menu (fzf-tab compatible format).
# See https://github.com/Aloxaf/fzf-tab/issues/43
...
zstyle -e ':completion:*:(ssh|scp|sftp|rsh|rsync):hosts' \
    hosts 'reply=(${=${${(f)"$(cat {/etc/ssh_,${XDG_CONFIG_HOME:-$HOME/.config}/ssh/known_}hosts(|2)(N) /dev/null)"}%%[# ]*}//,/ })'
